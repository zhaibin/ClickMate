...
class MouseControllerService {
  ...
  MouseControllerService(
    this._bindings, {
    Duration hotkeyArmDelay = const Duration(seconds: 1),
  }) {
    _hotkeysReadyAt = DateTime.now().add(hotkeyArmDelay);
    print('========================================');
    ...
    try {
      ...
      print(
        '✓ Hotkey system initialization: ${initSuccess ? "Success" : "Failed"}',
      );

      if (initSuccess) {
        // Register hotkey 1: Start/Stop
        bool regToggle = _bindings.registerHotkey(
          hotkeyIdToggle,
          currentToggleHotkeyCode,
        );
        print(
          '${regToggle ? "✓" : "×"} Hotkey 1 [Start/Stop] $_modifierDisplay+1 (VK:0x${currentToggleHotkeyCode.toRadixString(16).toUpperCase()}): ${regToggle ? "Success" : "Failed"}',
        );

        // Register hotkey 2: Capture position
        bool regCapture = _bindings.registerHotkey(
          hotkeyIdCapture,
          currentCaptureHotkeyCode,
        );
        print(
          '${regCapture ? "✓" : "×"} Hotkey 2 [Capture] $_modifierDisplay+2 (VK:0x${currentCaptureHotkeyCode.toRadixString(16).toUpperCase()}): ${regCapture ? "Success" : "Failed"}',
        );
        ...
        if (regToggle || regCapture) {
          ...
          LoggerService.instance.warning(
            'All hotkey registration failed - may need administrator permission or hotkey conflict',
          );
        }
      } else {
        ...
        print(
          '  Solution: Ensure mouse_controller.dll is in application directory',
        );
        LoggerService.instance.error(
          'Hotkey system initialization failed - DLL load failed',
        );
      }
    } catch (e) {
  ...
  bool setToggleHotkey(int vkCode) {
    ...
    try {
      ...
      print(
        '${success ? "✓" : "×"} New hotkey register $_modifierDisplay+${_getKeyName(vkCode)} (VK:0x${vkCode.toRadixString(16).toUpperCase()}): ${success ? "Success" : "Failed"}',
      );
  ...
  void _startHotkeyCheck() {
    ...
    _hotkeyCheckTimer = Timer.periodic(const Duration(milliseconds: 100), (
      timer,
    ) {
      try {
        if (DateTime.now().isBefore(_hotkeysReadyAt)) {
          _bindings.checkHotkeyPressed(hotkeyIdToggle);
          _bindings.checkHotkeyPressed(hotkeyIdCapture);
          return;
        }
  ...
  void toggleAutoClick() async {
    print(
      '>>> Toggle click status (Current: ${_isRunning ? "Running" : "Stopped"})',
    );
    if (_isRunning) {
  ...
  void startAutoClick() {
    ...
    if (targetPosition == null) {
      ...
      print(
        '  Please set X and Y coordinates first (Click "Capture" button or manual input)',
      );
    ...
    if (enableAutoPauseResume) {
      print(
        '✓ Auto-pause enabled: deviation threshold ${deviationThreshold}px',
      );
    }
  ...
  void _performClick() {
    ...
    if (_isPausedByDeviation) {
      // Schedule next check
      _clickTimer = Timer(
        Duration(milliseconds: clickIntervalMs),
        _performClick,
      );
      return;
    }
    ...
    final intervalOffset = randomIntervalRange > 0
        ...
        : 0;
    final actualInterval = (clickIntervalMs + intervalOffset).clamp(
      10,
      1000000,
    );
    ...
    if (shouldLog) {
      print(
        '[Click #${clickCount + 1}] Position:($targetX,$targetY) Interval:${actualInterval}ms',
      );
    }
  ...
  void _startPositionMonitoring() {
    ...
    _positionMonitorTimer = Timer.periodic(const Duration(milliseconds: 200), (
      timer,
    ) {
      if (!_isRunning || targetPosition == null) {
        _stopPositionMonitoring();
      ...
      if (_lastMonitoredPosition != null) {
        final moveDistance = _calculateDistance(
          currentPos,
          _lastMonitoredPosition!,
        );
        if (moveDistance > 5) {
          // Mouse moved more than 5 pixels
          _lastMoveTime = DateTime.now();
      ...
      if (distance > deviationThreshold) {
        ...
        if (!_isPausedByDeviation) {
          ...
          print(
            '⏸ Auto-paused: Mouse deviated ${distance.toInt()}px from target (threshold: ${deviationThreshold}px)',
          );
      ...
      if (_isPausedByDeviation) {
        ...
        print(
          '[DEBUG] Paused - distance=${distance.toInt()}px, idleTime=${timeSinceLastMove.inSeconds}s, required=${resumeDelaySeconds}s',
        );

        if (timeSinceLastMove.inSeconds >= resumeDelaySeconds) {
          // Mouse has been idle long enough at ANY position
          print(
            'Mouse idle for ${timeSinceLastMove.inSeconds}s at any position, moving to target and resuming...',
          );
          print(
            '[DEBUG] Target position: (${targetPosition!.x}, ${targetPosition!.y})',
          );
          print(
            '[DEBUG] Current position before move: (${currentPos.x}, ${currentPos.y})',
          );
          ...
          Future.delayed(const Duration(milliseconds: 200), () {
            ...
            print(
              '[DEBUG] Position after move: (${posAfterMove.x}, ${posAfterMove.y})',
            );
            final distanceAfterMove = _calculateDistance(
              posAfterMove,
              targetPosition!,
            );
            print(
              '[DEBUG] Distance after move: ${distanceAfterMove.toInt()}px',
            );
          ...
          final remainingSeconds =
              resumeDelaySeconds - timeSinceLastMove.inSeconds;
          print(
            'Mouse idle at (${currentPos.x}, ${currentPos.y}), will resume in ${remainingSeconds}s if no movement...',
          );
          ...
          _resumeTimer = Timer(Duration(seconds: remainingSeconds), () {
            ...
            final finalTimeSinceMove = DateTime.now().difference(
              _lastMoveTime!,
            );

            if (finalTimeSinceMove.inSeconds >= resumeDelaySeconds &&
                _isPausedByDeviation) {
              print(
                'Resume timer triggered: Mouse idle for ${finalTimeSinceMove.inSeconds}s',
              );
              print(
                '[DEBUG] Target position: (${targetPosition!.x}, ${targetPosition!.y})',
              );
              print(
                '[DEBUG] Current position before move: (${finalPos.x}, ${finalPos.y})',
              );
  ...
  void _resumeFromDeviation() {
    if (_isPausedByDeviation && _isRunning) {
      ...
      LoggerService.instance.info(
        'Auto-resumed after mouse returned to target',
      );
      onPauseStatusChanged?.call(false);
